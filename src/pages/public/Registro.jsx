...
export default function Registro() {
  ...
  const handleSubmit = async (e) => {
    ...
    // PASO 1: Construir datos del usuario (SIN campos de minimarket)
    const datosUsuario = {
    ...
    try {
      ...
      const responseUsuario = await registroService.registrar(datosUsuario);
      const usuarioId = responseUsuario.id;

      // Guardar token si el backend lo devuelve
      if (responseUsuario.token) {
        localStorage.setItem('authToken', responseUsuario.token);
      }

      // Si es admin, podrá crear su tienda desde el panel de administración
      ...
      if (window.Swal) {
        const mensajeAdmin = form.tipoUsuario === "admin"
          ? "Ahora puedes iniciar sesión y crear tu minimarket desde el panel de administración."
